...
//족보 판단
const judgePoker = (cards, hasEasyFlush) => {
  ...
  if (isStraight || isLowAceStraight) {
    ...
    if (isFlush) {
      return {
        pokerName: `${suits[0]}${getCardLabel(
          Number(highest)
        )} 스트레이트 플러시`,
        pokerScore: 100,
    ...
    return {
      pokerName: `${getCardLabel(Number(Math.max(...cardNumbers)))} 스트레이트`,
      pokerScore: 40,
      pokerCards: straightCards,
      multiplier: 4,
    };
  ...
  if (isFourCard) {
    ...
    return {
      pokerName: `${getCardLabel(Number(quad))} 포카드`,
      pokerScore: 80,
      pokerCards,
      multiplier: 10,
    };
  ...
  if (isFullHouse) {
    ...
    return {
      pokerName: `${getCardLabel(Number(triple))}, ${getCardLabel(
        Number(pair)
      )} 풀하우스`,
      pokerScore: 60,
      pokerCards,
  ...
  if (isFlush) {
    return {
      pokerName: `${hasEasyFlush ? "이지" : suits[0]} 플러시`,
      pokerScore: 50,
  ...
  if (isTriple) {
    ...
    return {
      pokerName: `${getCardLabel(Number(triple))} 트리플`,
      pokerScore: 30,
      pokerCards,
      multiplier: 3,
    };
  ...
  if (isTwoPair) {
    ...
    return {
      pokerName: `${getCardLabel(Number(pairs[0]))}, ${getCardLabel(
        Number(pairs[1])
      )} 투페어`,
      pokerScore: 20,
      pokerCards,
  ...
  if (isOnePair) {
    ...
    return {
      pokerName: `${getCardLabel(Number(pair))} 원페어`,
      pokerScore: 10,
      pokerCards,
      multiplier: 1,
    };
  ...
  return {
    pokerName: `${getCardLabel(highest)} 하이카드`,
    pokerScore: 5,
    pokerCards: [highest],
    multiplier: 1,
  };
...
export const calculateFinalScore = (cards, jokers) => {
  ...
  const { pokerName, pokerScore, pokerCards, multiplier } = judgePoker(
    adjustedCards,
    hasEasyFlush
  );
